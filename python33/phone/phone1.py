...
def delete_movie(movie_list):
    ...
    return movie_list 
movie_file = "friend.txt"
try:
    # try to autoload existing movie database file
    fin = open(movie_file, "rb")#allagh sto proto arxeio apo rb->wb->rb
    movie_list = []
    movie_list = pickle.load(fin)#gia arxeio pou leitourgei-> rd,movie_list=pickle.load(fin)1
    #pickle.dump(movie_list,fin)#gia kainourgio arxeio ->wb,pickle.dump kai ekso to movie_list=pickle.load(fin)
    fin.close()
...
while True:
    ...
    if option == '1':
        ...
        while True:
              ...
              id = raw_input("Δωσε διαφορετικο νουμερο: ")
              if id in id_list:
